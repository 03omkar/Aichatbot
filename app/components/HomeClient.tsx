...
document.addEventListener("mousedown",handler)
    ...
    const features=[{
        title:'Plug & Use',
        des:'Add the chatbot to your site with the script tag.'
    },
    {
        title:'Admin Controlled',
        des:'You controll exactly what Ai knows and answer.'
    },
    {
        title:'Alwasy Online',
        des:'Your customer gets instant support 24/7.'
    }]
    ...
    return(
        <div className='min-h-screen bg-gradient-to-br from-white to-zinc-50 text-zinc-900 overflow-x-hidden'>
            ...
            {open  && (
                    ...
                    exit={{opacity:0,y:-6}}
                    className='absolute right-0 mt-3 w-44 g-white rounded-xl shadow-x1 border border-zinc-200 overflow-hidden'>
                        <button className='w-full text-left px-4 py-3 text-sm hover:bg-zinc-100' onClick={()=>navigate.push("/dashboard")}>Dashboard</button>
                        <button className='block text-left px-4 py-3 text-sm text-red-600 hover:bg-zinc-100'onClick={handleLogout}>Logout</button>
                    </motion.div>
            )}
            </AnimatePresence>
                </div>:<button className='px-5 py-2 rounded-full bg-black text-white text-sm font-medium hover:bg-zinc-800 
            ...
            <section className='pt-36 pb-28 px-6'>
                <div className='max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-20 items-center'>
                    ...
                    >
                        <h1 className='text-4xl md:text5xl font-semibold leading-tight'>Ai Customer Support<br/>Built For Modern Webistes</h1>
                        <p className='mt-6 text-lg text-zinc-600 max-w-xl'>
                            AI-powered support that listens, understands, and responds instantly.
                            Get answers, solve issues, and move forward—without the wait.
                        </p>
                    ...
                    <div className='mt-16 grid grid-cols-1 md:grid-cols-3 gap-10'>
                        {features.map((f,index)=>(
                            ...
                            className='bg-white rounded-2xl p-8 shadow-lg border border-zinc-200'>
                                ...
                                <p className='mt-3 text-zinc-600 text-sm'>{f.des}</p>
            ...
            <footer className='py-10 text-center text-sm text-zinc-500'>&copy;{new Date().getFullYear()}
                 SupportAi. all right reserved
            </footer>
